...
func handleCall(cntxt context.Context, h *ari.ChannelHandle, cid string, cname string, channame string, tenant string) {
        ...
        if (transcript != noTranscription) && (transcript != noAudio) {
                ...
                if(emailTo != "-") {
                // send email if an address or list is provided at command line
...
func telnyxSendGroup(msg string) { // sent as group (max 8)
        ...
        payload := map[string]interface{}{
                ...
                //"subject": subj,
                "text": msg,
                //"media_urls": "https://portal.telsynergy.ca/main/img/logo_TelSynergy.png",
        }
...
func usage(slf string, wh string) {
        fmt.Printf("\n%v\n\nUsage :\n\n\t%v IP User Pass AppName Tenant [\"SMS Name\"|-] [SMS_To|List|-] [SMS_From|-] [callbacknum|nocallbacknum|-] [transcribe|translate|-] [email_To|List|-] [LogFile|-] [DTMF_Instructions_File|-] [Recording_Instructions_File|-] [Goodbye_Sound|-] [verbose|silent|-]\n\n\t%v 127.0.0.1 ariuser aripass ariapp 999 \"Sender Name\" +15555555555 +15556667777 callbacknum transcribe email@domain.suf /var/log/ari_log.txt dtmf_instuctions recording_instructions goodbye_sound_file silent\n\n\t%v 127.0.0.1 ariuser aripass ariapp 999 - - - - - - - - - - -\n\n", wh, slf, slf, slf)
}
