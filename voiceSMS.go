...
const oaiKey = "sk-proj-YourOpenAIKey"

// // when doing multiple instances with different command-line arguments, the env value does not always seem to be passed, so add explicitly as below
const telnyxKey = "KEYyouyTelnyxKey"
...
var ariStyle = "transcribe"                     //// transcribe, translate, transback or chat

var ariVoice = "-"

var emailTo = "-"                               //// address to send email to with attachment

var logfile = "/path/to/your/logs/log.txt" //// need to arrange log rotation
...
func getGPTChatss(question string, lng string) (string, error) {
...
func handleCall(cntxt context.Context, h *ari.ChannelHandle, cid string, cname string, channame string, tenant string) {
        ...
        transcript := oaiTranscribe(cntxt, h, fmt.Sprintf("%v%v.wav", recordingDir, recName), cname, cid)
        ...
        if (transcript != noTranscription) && (transcript != noAudio) {
                ...
                if(ariStyle == "chat") {
                        ...
                        chatresp, err := getGPTChatss(transcript, langTo)
                        if err != nil {
...
func parseLang(deflang string, incname string) string {
        ...
        if(strings.HasPrefix(incname,"lang=")) { ////check for the lang= prefix in the incoming CID name, as lang=Serbo-Croatian
...
func oaiTranscribe(cntxt context.Context, h *ari.ChannelHandle, fname string, cname string, cid string) string {
        ...
        if((ariStyle == "translate") || (ariStyle == "transback")) { //// translate to english
                ...
                if(langTo == "english") {
                        ...
                        trans, err := getGPTChatss(transcription.Text, langTo)
                        if err != nil {
...
func textToSpeech(cntxt context.Context, h *ari.ChannelHandle, text string) error {
    ...
    tmpFile, err := os.CreateTemp(tmpfilepath, fmt.Sprintf("%v.wav",tmpfilename))
    if err != nil {
        logit("Failed to create temp file", "error", fmt.Sprintf("%v", err))
        return err
    }
    //defer os.Remove(tmpFile.Name()) // Clean up temp file when done, not necsaary, since we are moving the file and deleting it after
    ...
    //// rename and chown 555 555 the file (strip)
    os.Rename(tmpfilepath+"/"+tmpfl,tmpfilepath+"/"+af+".wav")
    //// the 2 next lines should not be necessary
    //os.Chown(tmpfilepath+"/"+af+".wav",555,555)
    //os.Chmod(tmpfilepath+"/"+af+".wav",0644)
...
func usage(slf string, wh string) {
        fmt.Printf("\n%v\n\nUsage :\n\n\t%v IP User Pass AppName Tenant [\"SMS Name\"|-] [SMS_To|List|-] [SMS_From|-] [callbacknum|nocallbacknum|-] [en|fr|indonesian|-] [transcribe|translate|transback|-] [alloy|echo|fable|onyx|nova|shimmer||-] [email_To|List|-] [LogFile|-] [DTMF_Instructions_File|-] [Recording_Instructions_File|-] [Goodbye_Sound|-] [verbose|silent|-]\n\n\t%v 127.0.0.1 ariuser aripass ariapp 999 \"Sender Name\" +15555555555 +15556667777 callbacknum - transcribe shimmer email@domain.suf /var/log/ari_log.txt dtmf_instuctions recording_instructions goodbye_sound_file silent\n\n\t%v 127.0.0.1 ariuser aripass ariapp 999 - - - - - - - - - - - - -\n\n", wh, slf, slf, slf)
}
